...
1. 음악 app 사용자는 직관적이고 간단한 앱을 선호함

   - 앱의 전체적인 복잡성 낮추기
   - 직관적이고 간단한 탐색
   - 메인에서 너무 많은 데이터 보여주기 X (탭 없애기)

2. 컨텐츠를 효율적으로 찾을 수 있어야함
   - 강력한 필터 및 검색기능
3. 성능이 좋아야함
   - 이미지 및 fetch 최적화
   - 적절한 캐싱
...
- 앱
  ...
  - [x] 오디오 재생
    ...
    - [x] 트랙 - 플레이 리스트 마지막에 추가 후 stems의 stemType(sample, mr, solo, full 등)으로 컨텐츠에 맞게 플레이어 재생
    - [x] 번들 - 번들 클릭 시 현재 플레이 리스트를 번들의 곡 리스트로 초기화
